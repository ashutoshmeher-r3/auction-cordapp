...
import net.corda.samples.flows.IssueCashFlow;
...
public class FlowTests {
    ...
    @Test
    public void testCashIssueFlow() throws Exception {
        IssueCashFlow flow = new IssueCashFlow(Amount.parseCurrency("100 USD"));
        CordaFuture<SignedTransaction> future = a.startFlow(flow);
        network.runNetwork();
        SignedTransaction signedTransaction = future.get();
        Ledger ledger = (Ledger) signedTransaction.getTx().getOutput(0);
        assertNotNull(ledger);

        IssueCashFlow flow_1 = new IssueCashFlow(Amount.parseCurrency("200 USD"), c.getInfo().getLegalIdentities().get(0));
        CordaFuture<SignedTransaction> future_1 = a.startFlow(flow_1);
        network.runNetwork();
        SignedTransaction signedTransaction_1 = future.get();
        Ledger ledger_1 = (Ledger) signedTransaction_1.getTx().getOutput(0);
        assertNotNull(ledger_1);
    }
}